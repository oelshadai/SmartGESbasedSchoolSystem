...
import {
  ...
  ExternalLink, XCircle, CalendarRange,
} from 'lucide-react';
...
const StudentBills = ({ studentIdOverride }: { studentIdOverride?: string }) => {
  ...
  const fetchBills = async () => {
    ...
    try {
      const termData = await secureApiClient.get<Omit<BillsData, 'weekly_bills'>>(
        `/fees/term-bills/my-bills/${params}`
      );

      // Fetch weekly bills only for the student's own view (not parent override)
      let weeklyBills: WeeklyBillItem[] = [];
      if (!studentIdOverride) {
        try {
          const payData = await secureApiClient.get<{ bills: WeeklyBillItem[] }>(
            '/fees/student-payments/my-bills/'
          );
          weeklyBills = (payData.bills || []).filter(
            (b) => b.collection_frequency === 'WEEKLY'
          );
        } catch {
          // weekly bills endpoint unavailable — silently skip
        }
      }

      setData({ ...termData, weekly_bills: weeklyBills });
    } catch (e: any) {
  ...
  // Handle Paystack callback — supports both ?paystack_ref= (term) and ?ref= (weekly/student)
  useEffect(() => {
    const termRef = searchParams.get('paystack_ref');
    const stuRef = searchParams.get('ref');
    const ref = termRef || stuRef;
    if (!ref || !isAuthenticated) return;

    setSearchParams((prev) => {
      prev.delete('paystack_ref');
      prev.delete('ref');
      return prev;
    }, { replace: true });

    const verifyUrl = ref.startsWith('STU-')
    ...
    secureApiClient
      .get<{ success?: boolean; already_recorded?: boolean }>(verifyUrl)
      .then((r) => {
  ...
  const handlePayOnline = async (billId: number, billType: 'term' | 'weekly' = 'term') => {
    setPayingBillId(billId);
    setPayingBillType(billType);
    try {
      let result: { authorization_url: string };
      if (billType === 'weekly') {
        result = await secureApiClient.post<{ authorization_url: string }>(
          '/fees/student-payments/pay/',
          { bill_id: billId, bill_type: 'weekly' }
        );
      } else {
  ...
  const { term_bills, daily_fees, weekly_bills, summary } = data;
  ...
  // Group weekly bills by fee_type name
  const weeklyGrouped: Record<string, WeeklyBillItem[]> = {};
  weekly_bills.forEach((b) => {
    if (!weeklyGrouped[b.fee_type]) weeklyGrouped[b.fee_type] = [];
    weeklyGrouped[b.fee_type].push(b);
  });

  const weeklyOutstandingTotal = weekly_bills.reduce(
    (s, b) => s + (b.status !== 'PAID' && b.status !== 'WAIVED' ? b.balance : 0),
    0
  );
    ...
    <div className="space-y-6 p-4 sm:p-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 gap-3">
        {[
          ...
          { label: 'Balance Owing', value: fmt(summary.total_balance + weeklyOutstandingTotal), icon: <TrendingDown className="h-4 w-4" />, color: (summary.total_balance + weeklyOutstandingTotal) > 0 ? 'text-red-600' : 'text-green-600' },
          ...
          <Card key={c.label}>
            <CardContent className="p-3 sm:p-4">
              ...
              <div className={`text-base sm:text-lg font-bold ${c.color} truncate`}>{c.value}</div>
            </CardContent>
          </Card>
        ))}
      </div>
                ...
                <CardContent className="px-4 pb-4 pt-0">
                  <div className="space-y-2">
                    {bills.map((bill) => (
                      <div
                        key={bill.id}
                        className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 rounded-lg border bg-muted/20 px-4 py-3"
                      >
                        ...
                        <div className="flex sm:flex-col items-center sm:items-end gap-2 sm:gap-1.5 flex-wrap">
                          ...
                          {data.online_payments_enabled && (bill.status === 'UNPAID' || bill.status === 'PARTIAL') && (
                            <Button
                              ...
                              disabled={payingBillId === bill.id && payingBillType === 'term'}
                              onClick={() => handlePayOnline(bill.id, 'term')}
                            >
                              {payingBillId === bill.id && payingBillType === 'term'
      ...
      {/* Weekly Bills */}
      {weekly_bills.length > 0 && (
        <div className="space-y-4">
          <h2 className="text-base font-semibold flex items-center gap-2">
            <CalendarRange className="h-4 w-4 text-purple-500" /> Weekly Fees
          </h2>
          {Object.entries(weeklyGrouped).map(([feeTypeName, bills]) => {
            const totalBilled = bills.reduce((s, b) => s + b.amount_billed, 0);
            const totalPaid = bills.reduce((s, b) => s + b.amount_paid, 0);
            const totalBalance = bills.reduce((s, b) => s + b.balance, 0);
            const outstanding = bills.filter((b) => b.status !== 'PAID' && b.status !== 'WAIVED');
            return (
              <Card key={feeTypeName}>
                <CardHeader className="pb-2 pt-4 px-4">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 sm:gap-2">
                    ...
                    <div className="flex flex-wrap gap-2 sm:gap-3 text-xs text-muted-foreground">
                      <span>Billed: <strong className="text-foreground">{fmt(totalBilled)}</strong></span>
                      <span>Paid: <strong className="text-green-600">{fmt(totalPaid)}</strong></span>
                      {totalBalance > 0 && (
                        <span>Due: <strong className="text-red-600">{fmt(totalBalance)}</strong></span>
                      )}
                    </div>
                  </div>
                </CardHeader>
                <CardContent className="px-4 pb-4 pt-0 space-y-2">
                  {bills.map((bill) => (
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        <div className="font-medium text-sm">{bill.period}</div>
                        {bill.due_date && (
                          <div className="text-xs text-muted-foreground mt-0.5">
                            Due: {new Date(bill.due_date).toLocaleDateString()}
                          </div>
                      ...
                      <div className="flex sm:flex-col items-center sm:items-end gap-2 sm:gap-1.5 flex-wrap">
                        ...
                        {data.online_payments_enabled && (bill.status === 'UNPAID' || bill.status === 'PARTIAL') && (
                          <Button
                            ...
                            disabled={payingBillId === bill.id && payingBillType === 'weekly'}
                            onClick={() => handlePayOnline(bill.id, 'weekly')}
                          >
                            {payingBillId === bill.id && payingBillType === 'weekly'
                              ? <Loader2 className="h-3 w-3 animate-spin" />
                              : <ExternalLink className="h-3 w-3" />}
                            Pay Online
                          </Button>
                  ...
                  {outstanding.length === 0 && (
                    <p className="text-xs text-green-600 text-center py-2">All weeks paid ✓</p>
                  )}
                </CardContent>
              </Card>
            );
      ...
      {term_bills.length === 0 && daily_fees.length === 0 && weekly_bills.length === 0 && (
        <Card>
