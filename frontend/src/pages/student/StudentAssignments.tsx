...
const StudentAssignments = () => {
  ...
  return (
    ...
    <div className="p-4 sm:p-6 space-y-4 sm:space-y-6 animate-fade-in">
      <div>
        <h1 className="text-xl sm:text-2xl font-bold text-foreground">My Assignments</h1>
          ...
          {assignments.map((assignment) => {
            ...
            return (
              <div key={assignment.id} className="animated-assignment-card">
                <div 
                  className="animated-assignment-card-content cursor-pointer hover:bg-muted/20 transition-colors p-3 sm:p-4"
                  onClick={() => handleStartAssignment(assignment)}
                >
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <div className="flex items-start gap-3 flex-1 min-w-0">
                      <div className="p-2 sm:p-2.5 rounded-lg bg-muted text-muted-foreground shrink-0 mt-0.5">
                        {sc.icon}
                      </div>
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-xs text-muted-foreground mt-0.5">
                          Due: {new Date(assignment.due_date).toLocaleDateString()} · Max: {assignment.points} pts
                          {assignment.time_limit && <> · {assignment.time_limit} min</>}
                        </p>
                    ...
                    <div className="flex items-center justify-between sm:justify-end gap-2 sm:gap-3 pl-9 sm:pl-0">
                      <div className="flex items-center gap-2">
                        ...
                        <Badge variant="outline" className={`text-[10px] sm:text-xs ${sc.class}`}>
                          {sc.label}
                        </Badge>
