...
export default function PayrollManagement() {
  ...
  const fetchPayroll = async () => {
    try {
      ...
      const payrollData = Array.isArray(response) ? response : response?.results || response?.data || [];
      ...
      <Card>
        ...
        <CardContent>
          <div className="flex gap-4 items-end">
            <div>
              <label className="theme-label block mb-2">Month</label>
              <select
            ...
            <div>
              <label className="theme-label block mb-2">Year</label>
              <select
                className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 theme-input"
                value={year}
            ...
            <Button onClick={generatePayroll} disabled={generating} className="theme-button">
              ...
              <tbody>
                {payroll.map((record) => (
                  <tr key={record.id} className="border-b">
                    ...
                    <td className="theme-table-cell text-right p-2">₵{record.basic_salary.toLocaleString()}</td>
                    <td className="theme-table-cell text-right p-2">₵{record.allowances.toLocaleString()}</td>
                    <td className="theme-table-cell text-right p-2">₵{record.deductions.toLocaleString()}</td>
                    <td className="theme-table-cell text-right p-2 font-bold">₵{record.net_salary.toLocaleString()}</td>
                    <td className="p-2">
                    ...
                    <td className="p-2">
                      <div className="flex gap-2">
                        {record.status === 'DRAFT' && (
                          <Button size="sm" onClick={() => approvePayroll(record.id)} className="theme-button">
                            Approve
                          </Button>
                        )}
                        {record.status === 'APPROVED' && (
                          <Button size="sm" onClick={() => markPaid(record.id)} className="theme-button">
