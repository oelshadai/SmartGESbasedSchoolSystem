  ...
  return (
    <div className="min-h-[100dvh] bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 flex flex-col overflow-y-auto px-3 py-4 sm:px-4 sm:py-6 relative">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        <div className="absolute -top-32 -right-32 w-64 sm:w-80 h-64 sm:h-80 bg-cyan-500/10 rounded-full blur-3xl animate-pulse" />
        <div className="absolute -bottom-32 -left-32 w-64 sm:w-80 h-64 sm:h-80 bg-sky-500/10 rounded-full blur-3xl animate-pulse" style={{ animationDelay: '1s' }} />
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-72 sm:w-96 h-72 sm:h-96 bg-sky-500/5 rounded-full blur-3xl" />
      </div>
            ...
            <div className="login-glow-card w-full max-w-md">
              <div className="login-glow-card-inner p-3 sm:p-8">
                ...
                <div className="mb-2 sm:mb-5">
                  <Label className="text-slate-100 text-[9px] sm:text-xs font-medium mb-1 sm:mb-2.5 block tracking-wide uppercase">Select Your Role</Label>
                      ...
                      return (
                        <button
                          ...
                          className={`relative group px-1 py-1 sm:py-3 rounded-xl transition-all duration-200 ${
                            isActive
                              ? 'bg-gradient-to-br from-cyan-500 to-sky-500 shadow-lg shadow-cyan-500/25 scale-[1.02]'
                              : 'bg-slate-800/60 hover:bg-slate-800 border border-slate-700/40 hover:border-slate-600/60'
                          }`}
                        >
                          <div className="flex flex-col items-center gap-0.5 sm:gap-1">
                            <RoleIcon className={`h-3.5 w-3.5 sm:h-4 sm:w-4 ${isActive ? 'text-white' : 'text-slate-100 group-hover:text-slate-300'}`} />
                            <span className={`text-[9px] sm:text-[11px] font-medium ${isActive ? 'text-white' : 'text-slate-100 group-hover:text-slate-300'}`}>
                              {role.label}
                            </span>
                          </div>
                        </button>
                  ...
                  <p className="hidden sm:block text-[10px] sm:text-[11px] text-slate-100 mt-1.5 text-center">{currentRole.description}</p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-1.5 sm:space-y-4">
                  ...
                  <div>
                    <Label htmlFor="identifier" className="text-slate-100 text-[10px] sm:text-sm font-medium block mb-0.5">
                      {currentRole.inputType === 'studentId' ? 'Student ID' : 'Email Address'}
                    </Label>
                    <div className="relative">
                      <Icon className="absolute left-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 sm:h-4 sm:w-4 text-slate-100" />
                      <Input
                        id="identifier"
                        ...
                        required
                        className="pl-9 h-8 sm:h-11 bg-slate-800/50 border-slate-700/50 text-white placeholder:text-slate-100 focus:border-orange-500 focus:ring-orange-500/20 rounded-xl text-xs sm:text-sm"
                      />
                    </div>
                  </div>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-0.5">
                      <Label htmlFor="password" className="text-slate-100 text-[10px] sm:text-sm font-medium">Password</Label>
                      {loginRole === 'student' || loginRole === 'parent' ? (
                        <span className="text-[9px] sm:text-[11px] text-slate-100">Contact admin to reset</span>
                      ) : (
                        <button
                          type="button"
                          onClick={() => { setShowForgot(true); setForgotMessage(''); setForgotError(''); }}
                          className="text-[9px] sm:text-[11px] text-cyan-300 hover:text-cyan-200 transition-colors"
                        >
                          Forgot Password?
                        </button>
                      )}
                    ...
                    <div className="relative">
                      <Lock className="absolute left-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 sm:h-4 sm:w-4 text-slate-100" />
                      <Input
                        id="password"
                        ...
                        required
                        className="pl-9 pr-9 h-8 sm:h-11 bg-slate-800/50 border-slate-700/50 text-white placeholder:text-slate-100 focus:border-orange-500 focus:ring-orange-500/20 rounded-xl text-xs sm:text-sm"
                      />
                      <button
                        type="button"
                        onClick={() => setShowPassword(!showPassword)}
                        className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-100 hover:text-slate-100 transition-colors"
                      >
                ...
                <div className="relative my-1.5 sm:my-5">
                  ...
                  <div className="relative flex justify-center text-[10px]">
                    <span className="bg-[hsl(222_47%_8%)] px-3 text-slate-100">New to the platform?</span>
                  </div>
                </div>
                ...
                <Button
                  ...
                  className="w-full h-7 sm:h-11 bg-transparent border-sky-500/30 text-slate-100 hover:bg-slate-800/60 hover:text-white hover:border-sky-400 rounded-xl transition-all duration-200 text-[10px] sm:text-sm"
                >
                ...
                <div className="mt-1.5 sm:mt-5 text-center hidden sm:block">
                  <p className="text-[10px] text-slate-100">
                    ...
                    <a href="#" className="text-slate-100 hover:text-slate-100 transition-colors">Terms</a>
                    {' '}and{' '}
                    <a href="#" className="text-slate-100 hover:text-slate-100 transition-colors">Privacy Policy</a>
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="bg-slate-900 border border-slate-700/60 rounded-2xl p-5 sm:p-6 w-full max-w-sm shadow-2xl shadow-black/50">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setShowForgot(false)} className="text-slate-100 hover:text-white transition-colors p-1">
                <X className="h-5 w-5" />
              </button>
            </div>
            <p className="text-slate-100 text-xs sm:text-sm mb-4">Enter your email address and we'll send you a reset link.</p>
            {forgotMessage ? (
              <div className="bg-green-950/50 border border-green-800 rounded-lg p-3 text-green-300 text-sm">{forgotMessage}</div>
            ) : (
              <form onSubmit={handleForgotPassword} className="space-y-4">
                <div>
                  <Label htmlFor="forgot-email" className="text-slate-100 text-sm">Email Address</Label>
                  <Input
                    id="forgot-email"
                    type="email"
                    ...
                    required
                    className="mt-1 bg-slate-800/50 border-slate-700 text-white placeholder:text-slate-100 focus:border-orange-500"
                  />
                </div>
