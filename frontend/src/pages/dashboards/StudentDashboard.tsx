...
const STATUS_BADGE: Record<string, string> = {
  ...
  NOT_STARTED: 'bg-muted text-muted-foreground/80',
...
const StudentDashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      try {
        ...
        // Fetch fee summary — use student-payments endpoint so weekly bills are included
        secureApiClient.get<any>('/fees/student-payments/my-bills/').then((d) => {
          if (d?.summary) {
            // student-payments returns total_outstanding + total_paid
            setFeeSummary({
              total_billed: d.summary.total_outstanding + d.summary.total_paid,
              total_balance: d.summary.total_outstanding,
              total_daily_paid: 0,
              grand_total_paid: d.summary.total_paid,
            });
          } else {
            // fallback to term-bills endpoint
            secureApiClient.get<any>('/fees/term-bills/my-bills/').then((td) => {
              if (td?.summary) setFeeSummary(td.summary);
            }).catch(() => null);
          }
        }).catch(() => {
          secureApiClient.get<any>('/fees/term-bills/my-bills/').then((td) => {
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="text-center">
          ...
          <p className="text-foreground/70 text-sm">Loading dashboard…</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-w-0 w-full max-w-full p-3 sm:p-6 overflow-x-hidden overflow-y-auto">
    <div className="min-w-0 w-full max-w-full space-y-4 sm:space-y-6 animate-fade-in">
      ...
      <div className="flex items-center gap-3 sm:gap-4">
        ...
        <div className="min-w-0">
          ...
          <p className="text-xs sm:text-sm text-foreground/70 truncate">{student.school} · {student.class}</p>
        </div>
      </div>
      ...
      {feeSummary !== null && (
        <div className="rounded-2xl border bg-gradient-to-r from-blue-950/40 to-indigo-950/40 border-blue-800/40 p-4 sm:p-5">
          ...
          <div className="grid grid-cols-3 gap-2 sm:gap-3">
            <div className="text-center">
              <div className="text-[10px] sm:text-xs text-foreground/60 mb-0.5">Total Paid</div>
            ...
            <div className="text-center border-x border-blue-800/40">
              <div className="text-[10px] sm:text-xs text-foreground/60 mb-0.5">Balance Owing</div>
              <div className={`text-sm sm:text-base font-bold ${feeSummary.total_balance > 0 ? 'text-red-400' : 'text-green-400'}`}>
                GH₵ {feeSummary.total_balance.toLocaleString('en-GH', { minimumFractionDigits: 2 })}
              </div>
            </div>
            <div className="text-center">
              <div className="text-[10px] sm:text-xs text-foreground/60 mb-0.5">Daily Paid</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
        ...
        <div className="md:col-span-2 rounded-xl border bg-card shadow-sm">
          <div className="p-4 sm:p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-xs text-foreground/60">{assignments.length} total</span>
            </div>
            {recentAssignments.length === 0 ? (
              <div className="flex flex-col items-center py-8 gap-2">
                <ClipboardList className="h-10 w-10 text-foreground/50" />
                <p className="text-sm text-foreground/60">No assignments yet</p>
              </div>
            ) : (
              <div className="space-y-2">
                  ...
                  <div key={a.id} className="flex items-start justify-between gap-3 p-3 rounded-lg bg-muted/40 hover:bg-muted/70 transition-colors">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                        {a.subject_name && <span className="text-xs text-foreground/60">{a.subject_name}</span>}
                        <span className="text-xs text-foreground/40">·</span>
                        <span className="text-xs text-foreground/60">{TYPE_LABEL[a.assignment_type] ?? a.assignment_type}</span>
                    ...
                    <Badge className={`text-[10px] border-0 shrink-0 ${STATUS_BADGE[a.status] ?? 'bg-muted text-muted-foreground/80'}`}>
                      {a.status.replace('_', ' ')}
                    </Badge>
                  </div>
                ))}
        ...
        <div className="rounded-xl border bg-card shadow-sm">
          <div className="p-4 sm:p-5">
            ...
            {classmates.length === 0 ? (
              <p className="text-sm text-foreground/60 text-center py-6">No classmates found</p>
            ) : (
              <div className="space-y-2">
                {classmates.map((c) => (
                  <div key={c.id} className="flex items-center gap-3 p-2 rounded-lg hover:bg-muted/50 transition-colors">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-foreground/70">{c.student_id}</p>
                    </div>
                  </div>
                ))}
              </div>
      ...
      <div className="rounded-xl border bg-card shadow-sm">
        <div className="p-4 sm:p-5">
          ...
          {announcements.length === 0 ? (
            <div className="flex flex-col items-center py-8 gap-2">
              <Bell className="h-10 w-10 text-foreground/40" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {announcements.map((ann) => (
                <div key={ann.id} className={`rounded-xl p-4 border transition-colors ${
                  ...
                  <p className="text-xs text-foreground/70 leading-relaxed line-clamp-3">{ann.content}</p>
