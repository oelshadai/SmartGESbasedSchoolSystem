...
export default function AdminAnalytics() {
  ...
  const GradientBar = ({
    ...
    if (!items?.length) return <p className="text-muted-foreground text-sm py-4 text-center">No data available.</p>;
        ...
        {trimmed.map((item, i) => {
          ...
          return (
            <div key={i} className="flex-1 flex flex-col items-center gap-1 min-w-0 group">
              ...
              <span className="text-[9px] text-muted-foreground group-hover:text-foreground transition-colors truncate w-full text-center leading-none">
    ...
    <div className="space-y-6 p-4 sm:p-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">Platform-wide usage and revenue metrics</p>
        </div>
          ...
          <div className="relative">
            ...
            <div className="space-y-2">
              {expiring_soon.map((s: any) => (
                <div key={s.id} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 py-3 px-4 rounded-xl bg-slate-800/50 border border-slate-700/40 hover:border-yellow-500/20 transition-colors">
                <div>
