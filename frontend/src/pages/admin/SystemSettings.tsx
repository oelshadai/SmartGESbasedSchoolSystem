...
const SystemSettings = () => {
  ...
  const FieldInput = ({ label, value, onChange, type = 'text', placeholder = '', span2 = false }: {
    ...
    <div className={span2 ? 'md:col-span-2' : ''}>
      <Label className="text-foreground/60 text-sm font-medium">{label}</Label>
      <Input
        type={type}
        value={value}
        onChange={e => onChange(e.target.value)}
        placeholder={placeholder}
        className="mt-2 h-11 bg-slate-800/50 border-slate-700/50 text-white placeholder:text-foreground/70 focus:border-cyan-500/50 focus:ring-cyan-500/20"
      />
    </div>
  );
  ...
  const SwitchRow = ({ label, checked, onChange }: { label: string; checked: boolean; onChange: (v: boolean) => void }) => (
    <div className="flex items-center justify-between p-3 bg-slate-800/30 hover:bg-slate-800/50 rounded-xl transition-colors">
      <Label className="text-foreground/60 text-sm cursor-pointer">{label}</Label>
    ...
    <div className="relative group bg-slate-900/50 backdrop-blur-xl rounded-2xl border border-slate-800/50 p-6 hover:border-slate-700/50 transition-all duration-300 overflow-hidden">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-foreground/70 text-xs">{subtitle}</p>
        </div>
      </div>
      {children}
    </div>
      ...
      <div className="relative p-4 sm:p-6 space-y-6 sm:space-y-8">
        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-foreground/70 text-sm pl-12">Configure school preferences, academic setup, and report options</p>
          </div>
          <Button
            onClick={handleSave}
            disabled={saving}
            ...
            <div key={stat.label} className={`relative group rounded-2xl border ${stat.border} bg-slate-900/60 backdrop-blur-xl p-5 shadow-xl ${stat.glow} hover:scale-[1.02] transition-all duration-300 overflow-hidden`}>
              ...
              <div className="relative flex items-start justify-between">
                <div className="flex-1 min-w-0">
                  <p className="text-foreground/70 text-xs mb-1.5">{stat.label}</p>
        ...
        <Tabs defaultValue="profile" className="space-y-6">
          <TabsList className="bg-slate-900/60 backdrop-blur-xl border border-slate-800/50 p-1 rounded-xl h-auto flex-wrap">
            <TabsTrigger value="profile" className="data-[state=active]:bg-gradient-to-r data-[state=active]:from-blue-600 data-[state=active]:to-cyan-600 data-[state=active]:text-white text-foreground/70 rounded-lg px-4 py-2 text-sm gap-1.5">
              <School className="h-3.5 w-3.5" />School Profile
            </TabsTrigger>
            <TabsTrigger value="academic" className="data-[state=active]:bg-gradient-to-r data-[state=active]:from-blue-600 data-[state=active]:to-cyan-600 data-[state=active]:text-white text-foreground/70 rounded-lg px-4 py-2 text-sm gap-1.5">
              <BookOpen className="h-3.5 w-3.5" />Academic
            </TabsTrigger>
            <TabsTrigger value="reports" className="data-[state=active]:bg-gradient-to-r data-[state=active]:from-blue-600 data-[state=active]:to-cyan-600 data-[state=active]:text-white text-foreground/70 rounded-lg px-4 py-2 text-sm gap-1.5">
              <FileText className="h-3.5 w-3.5" />Reports
            </TabsTrigger>
            <TabsTrigger value="grading" className="data-[state=active]:bg-gradient-to-r data-[state=active]:from-blue-600 data-[state=active]:to-cyan-600 data-[state=active]:text-white text-foreground/70 rounded-lg px-4 py-2 text-sm gap-1.5">
          ...
          <TabsContent value="profile">
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <Label className="text-foreground/60 text-sm font-medium">School Logo</Label>
                  ...
                  {settings.logo && (
                    <p className="text-xs text-foreground/70 mt-1 truncate">Current: {settings.logo.split('/').pop()}</p>
                  )}
                </div>
                <div>
                  <Label className="text-foreground/60 text-sm font-medium">Principal Signature</Label>
                  <Input type="file" accept="image/*" className="mt-2 h-11 bg-slate-800/50 border-slate-700/50 text-white file:mr-3 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:bg-slate-700 file:text-white file:text-xs" />
                  {settings.principal_signature && (
                    <p className="text-xs text-foreground/70 mt-1 truncate">Current: {settings.principal_signature.split('/').pop()}</p>
                  )}
                </div>
              </div>
            </SectionCard>
          ...
          <TabsContent value="academic">
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                />
                <div>
                  <Label className="text-foreground/60 text-sm font-medium">Current Term</Label>
                  <Select
                ...
                <div>
                  <Label className="text-foreground/60 text-sm font-medium">Score Entry Mode</Label>
          ...
          <TabsContent value="reports">
            ...
            >
              <div className="space-y-5">
                <div>
                  <Label className="text-foreground/60 text-sm font-medium">Report Template</Label>
          ...
          <TabsContent value="grading">
            ...
            >
              <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                {[
                  ...
                  <div key={g.key} className={`rounded-xl border p-4 ${g.bg} text-center`}>
                    ...
                    />
                    <p className="text-foreground/70 text-xs mt-1.5">min score</p>
                  </div>
                ))}
              </div>
              ...
              <div className="mt-6 p-4 bg-slate-800/30 rounded-xl">
                <p className="text-foreground/70 text-xs font-medium mb-3">Grade Distribution Preview</p>
                ...
                <div className="flex justify-between text-foreground/70 text-xs mt-1">
