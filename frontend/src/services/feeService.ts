...
export interface GenerateWeeklyBillsInput {
  start_date: string;
  end_date: string;
  fee_type?: number | null;
  overwrite?: boolean;
...
class FeeService {
  ...
  async generateWeeklyBills(data: GenerateWeeklyBillsInput): Promise<GenerateBillsResult> {
    const result = await secureApiClient.post<GenerateBillsResult>('/fees/weekly-bills/generate/', data);
    return result;
  }
