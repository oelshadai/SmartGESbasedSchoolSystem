    ...
    <div className={`dashboard-pop-card relative group rounded-2xl border ${style.border} bg-card/60 backdrop-blur-sm p-3.5 sm:p-5 shadow-lg ${style.shadow} hover:scale-[1.02] transition-all duration-200 overflow-hidden`}>
      ...
      <div className="relative flex items-start justify-between">
        <div className="flex-1 min-w-0">
          <p className="text-xs font-medium text-muted-foreground mb-1.5 sm:mb-2">{label}</p>
          <p className="text-2xl sm:text-3xl font-bold text-foreground leading-none tabular-nums">{displayValue}</p>
          {trend && <p className="text-[10px] sm:text-xs text-emerald-500 mt-1.5 sm:mt-2 font-medium">{trend}</p>}
        </div>
        <div className={`flex-shrink-0 p-2 sm:p-2.5 rounded-xl ${style.bg} border ${style.border} ${color}`}>
          {icon}
        </div>
      </div>
    </div>
