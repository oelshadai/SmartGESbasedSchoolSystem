...
const getNavItems = (role: UserRole): NavItem[] => {
  const base: Record<UserRole, NavItem[]> = {
    SUPER_ADMIN: [
      ...
      { label: 'Billing',        path: '/admin/billing',       icon: <DollarSign className="h-5 w-5" /> },
      { label: 'Audit Logs',     path: '/admin/audit-logs',    icon: <Shield className="h-5 w-5" /> },
      { label: 'System Health',  path: '/admin/system-health', icon: <Activity className="h-5 w-5" /> },
      { label: 'Messages',       path: '/admin/messages',      icon: <MessageSquare className="h-5 w-5" /> },
      { label: 'Support',        path: '/admin/support',       icon: <HelpCircle className="h-5 w-5" /> },
    ],
    SCHOOL_ADMIN: [
      ...
      { label: 'Messages', path: '/school/messages', icon: <MessageSquare className="h-5 w-5" /> },
    ...
    <div className="flex w-full h-screen overflow-hidden bg-background professional-3d page-shell">
      ...
      <div className="flex-1 min-w-0 w-full flex flex-col overflow-hidden">
        <header className="h-14 sm:h-16 border-b border-border flex items-center justify-between px-4 sm:px-6 bg-card/50 backdrop-blur-sm">
          <button onClick={() => setMobileOpen(prev => !prev)} className="lg:hidden p-2.5 -ml-2 text-muted-foreground hover:text-foreground rounded-lg hover:bg-muted active:bg-muted/80 transition-colors" aria-label={mobileOpen ? 'Close sidebar' : 'Open sidebar'}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative" ref={notifRef}>
              ...
              {/* Notification Panel */}
              {notifOpen && (
                <div className="absolute right-0 top-10 w-[calc(100vw-2rem)] max-w-sm sm:max-w-96 bg-card border border-border rounded-2xl shadow-2xl z-50 overflow-hidden">
                  {/* Panel Header */}
