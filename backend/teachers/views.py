...
class TeacherViewSet(viewsets.ModelViewSet):
    ...
    def credentials(self, request, pk=None):
        """Get teacher portal login credentials"""
        try:
            teacher = self.get_object()
            
            # Get teacher name - fallback if get_full_name fails
            try:
                teacher_name = teacher.get_full_name()
            except:
                teacher_name = f"{teacher.user.first_name} {teacher.user.last_name}".strip() if teacher.user else "Unknown"
            
            # Get assigned classes safely
            try:
                assigned_classes = teacher.get_assigned_classes()
                class_name = str(assigned_classes.first()) if assigned_classes.exists() else 'No class assigned'
            except:
                class_name = 'No class assigned'
            
            # Get email safely - email is on the User model, not Teacher
            try:
            ...
            return Response({
                ...
                'employee_id': teacher.employee_id or 'N/A',
                'email': email,
                'class_name': class_name,
                'portal_url': 'https://yourschool.com/teacher-login',
                'note': 'Use email as username and the initial password provided during account creation'
            })
        except Exception as e:
            import logging
            logger = logging.getLogger(__name__)
            logger.exception('Error in credentials endpoint')
            return Response(
                {'error': 'Failed to load teacher credentials', 'detail': str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
