...
class WeeklyBill(models.Model):
    """
    Weekly fee billing for students.
    Tracks weekly fee charges for WEEKLY collection frequency fee types.
    Each record represents a week's charge for a specific student + fee type combo.
    """
    BILL_STATUS_CHOICES = [
        ('UNPAID', 'Not Paid'),
        ('PARTIAL', 'Partially Paid'),
    ...
    student = models.ForeignKey(
        'students.Student', on_delete=models.CASCADE, related_name='weekly_bills'
    )
    school = models.ForeignKey(School, on_delete=models.CASCADE, related_name='weekly_bills')
    fee_type = models.ForeignKey(FeeType, on_delete=models.PROTECT, related_name='weekly_bills')
    ...
    def __str__(self):
        return f"{self.student.student_id} | {self.fee_type.name} | Week {self.week_start}"
