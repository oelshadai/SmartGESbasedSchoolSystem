...
from .serializers import (
    ...
    TermBillSerializer, GenerateBillsSerializer, GenerateWeeklyBillsSerializer,
    StudentFeeSubTypeSerializer,
    WeeklyBillSerializer, StudentInitiatePaymentSerializer,
)
...
class TermBillViewSet(viewsets.ModelViewSet):
    ...
    def summary(self, request):
        ...
        summary = qs.values('fee_type__name', 'status').annotate(
            count=Count('student', distinct=True),  # Count unique students, not bills
    ...
    def send_fee_reminders(self, request):
        ...
        Body (all optional):
          {
            ...
            "skip_already_messaged": false // if true, exclude students messaged in last 7 days
          }
        ...
        if fee_types:
            ...
            if not isinstance(fee_types, list):
                fee_types = [fee_types]
            qs = qs.filter(fee_type_id__in=fee_types)
        
        # Exclude guardians already messaged in last 7 days (skip entire guardian group)
        if skip_already_messaged:
            seven_days_ago = timezone.now() - timedelta(days=7)
            recently_messaged_logs = SmsLog.objects.filter(
                school=school,
            ...
            # Extract guardian phone numbers from details in recent logs
            recently_messaged_phones = set()
            for log in recently_messaged_logs:
                for detail in log.details:
                    if 'guardian_phone' in detail and detail['guardian_phone']:
                        recently_messaged_phones.add(detail['guardian_phone'])
            
            # Filter out bills for these phone numbers (entire guardian group)
            if recently_messaged_phones:
                qs = qs.exclude(student__guardian_phone__in=recently_messaged_phones)

        # Group bills by guardian_phone so each parent gets ONE SMS with all their children's fees
        from collections import defaultdict
        guardian_bills: dict = defaultdict(list)
        for bill in qs:
            phone = getattr(bill.student, 'guardian_phone', '') or ''
            guardian_bills[phone].append(bill)
        ...
        for phone, bills in guardian_bills.items():
            # Get unique students for this guardian
            unique_students = list(set(b.student for b in bills))
            student = unique_students[0]  # Primary student for reference
            ...
            if custom_message:
                sms_text = custom_message.replace('{student}', ' & '.join([s.get_full_name() for s in unique_students])).replace('{balance}', f'GH\u20b5{total_balance:,.2f}')
            else:
                guardian = getattr(student, 'guardian_name', '') or 'Guardian'
                school_name = school.name
                
                # List all students for this parent
                student_names = ', '.join([s.get_full_name() for s in unique_students])

                # Build per-student, per-fee breakdown
                student_sections = []
                for stud in unique_students:
                    stud_bills = [b for b in bills if b.student_id == stud.id]
                    fee_lines = []
                    stud_total_balance = sum(float(b.balance) for b in stud_bills)
                    for b in stud_bills:
                        billed = float(b.amount_billed)
                        paid = float(b.amount_paid)
                        arrears = float(b.balance)
                        fee_lines.append(
                            f'{b.fee_type.name}: GH\u20b5{arrears:,.2f}'
                        )
                    fees_summary = ', '.join(fee_lines)
                    student_sections.append(f'{stud.get_full_name()} ({fees_summary})')
                
                breakdown = ' | '.join(student_sections)

                sms_text = (
                    f'Dear {guardian}, fee reminder from {school_name}.\n'
                    f'{breakdown}\n'
                    f'Total Outstanding: GH\u20b5{total_balance:,.2f}\n'
                    f'Please settle fees. Thank you.'
                )
            ...
            detail = {
                'student': ', '.join([s.get_full_name() for s in unique_students]),  # All students for this parent
                'student_ids': [s.student_id for s in unique_students],
            ...
            # Also send in-app notifications to linked parent user accounts for all students
            try:
                from accounts.models import ParentStudent
                parent_users_set = set()
                for stud in unique_students:
                    parent_users = [ps.parent for ps in ParentStudent.objects.filter(student=stud).select_related('parent')]
                    parent_users_set.update(parent_users)
                
                # Create consolidated notification for each parent
                student_names_msg = ', '.join([s.get_full_name() for s in unique_students])
                for parent in parent_users_set:
                    Notification.objects.create(
                        user=parent,
                        title='📢 Fee Payment Reminder',
                        message=f'Outstanding balance for {student_names_msg}: GH₵{total_balance:,.2f}. Please clear fees as soon as possible.',
                        type='warning',
                    )
                
                # Also notify each student's own user account if it exists
                for stud in unique_students:
                ...
                logger.warning(f'In-app notification failed for guardian {phone}: {notif_err}')
        ...
        if not dry_run:
            try:
                ...
                SmsLog.objects.create(
                    ...
                    sms_type='fee_reminder',
                    status=log_status,
                    total_recipients=len(guardian_bills),
                    sent_count=sent,
        ...
        try:
            ...
            # Also refresh the in-memory school object for consistency
        ...
        return Response({
            ...
            'skipped': skipped,
            'no_phone': no_phone,
            'total_parents': len(guardian_bills),
...
# ---------------------------------------------------------------------------
# Weekly Bills ViewSet (admin/teacher management)
# ---------------------------------------------------------------------------

class WeeklyBillViewSet(viewsets.ReadOnlyModelViewSet):
    """View weekly fee bills"""
    serializer_class = WeeklyBillSerializer
    permission_classes = [permissions.IsAuthenticated]

    def get_queryset(self):
        qs = WeeklyBill.objects.filter(school=self.request.user.school).select_related(
            'student__user', 'student__current_class', 'fee_type'
        )
        student = self.request.query_params.get('student')
        if student:
    ...
    @action(detail=False, methods=['post'])
    def generate(self, request):
        """Bulk-generate WeeklyBills for all active students in a week range."""
        serializer = GenerateWeeklyBillsSerializer(data=request.data)
        if not serializer.is_valid():
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
        ...
        if data.get('fee_type'):
            ...
            fee_types = list(FeeType.objects.filter(
                school=school, is_active=True, collection_frequency='WEEKLY'
            ))

        if not fee_types:
            return Response({'error': 'No eligible WEEKLY fee types found.'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        for fee_type in fee_types:
            for student in students:
                ...
                existing = WeeklyBill.objects.filter(
                    student=student,
                    fee_type=fee_type,
                ...
                if existing:
                    if data.get('overwrite', False):
                        existing.amount_billed = structure.amount
                        existing.week_end = data['end_date']
                        existing.save()
                        updated_count += 1
                    else:
                        skipped_count += 1
                else:
                    WeeklyBill.objects.create(
                        student=student,
                        school=school,
                        fee_type=fee_type,
                        week_start=data['start_date'],
        ...
        return Response({
            ...
            'skipped': skipped_count,
            'message': (
                f'{created_count} weekly bills created, {updated_count} updated, '
...
# ---------------------------------------------------------------------------
# Student Online Payment Initiation
# ---------------------------------------------------------------------------

class StudentPaymentViewSet(viewsets.ViewSet):
    """
    ViewSet for students to view their bills and pay online via Paystack.
    
    Endpoints:
      GET    /fees/student-payments/my-bills/   – list all bills (term + weekly)
      POST   /fees/student-payments/pay/        – initiate Paystack payment
      GET    /fees/student-payments/verify/     – verify Paystack payment
    """
    permission_classes = [permissions.IsAuthenticated]
    ...
    def _get_student(self, request):
        ...
        role = getattr(request.user, 'role', '')
        if role != 'STUDENT':
            return None, Response({'error': 'Only students can access this endpoint.'}, status=status.HTTP_403_FORBIDDEN)
        try:
            return request.user.student_profile, None
        except Exception:
            return None, Response({'error': 'Student record not found.'}, status=status.HTTP_404_NOT_FOUND)

    def _check_online_enabled(self, school):
        """Check if student online payments are enabled for this school."""
        if not getattr(school, 'student_can_pay_online', False):
            return Response({'error': 'Online payments are not enabled for students at this school.'}, status=status.HTTP_400_BAD_REQUEST)
        secret_key = getattr(django_settings, 'PAYSTACK_SECRET_KEY', '')
        if not secret_key:
            return Response({'error': 'Payment gateway not configured.'}, status=status.HTTP_400_BAD_REQUEST)
        return None
    ...
    def my_bills(self, request):
        """
        GET /fees/student-payments/my-bills/
        Returns all outstanding bills (term + weekly) for the authenticated student.
        """
        student, err_response = self._get_student(request)
        if err_response:
            return err_response
        ...
        # --- Term bills ---
        term_bills_qs = TermBill.objects.filter(
            student=student, school=school
        ).select_related('fee_type', 'term', 'term__academic_year').order_by(
            '-term__start_date', 'fee_type__name'
        )
        ...
        term_bills_data = []
        for bill in term_bills_qs:
            term_bills_data.append({
                'id': bill.id,
                'type': 'term',
                'fee_type': bill.fee_type.name,
                'fee_type_id': bill.fee_type_id,
                'collection_frequency': bill.fee_type.collection_frequency,
                'period': f"{bill.term.get_name_display()} - {bill.term.academic_year.name}",
                'amount_billed': float(bill.amount_billed),
                'amount_paid': float(bill.amount_paid),
                'balance': float(bill.balance),
                'status': bill.status,
        ...
        # --- Weekly bills ---
        weekly_bills_qs = WeeklyBill.objects.filter(
            student=student, school=school
        ).select_related('fee_type').order_by('-week_start', 'fee_type__name')
        
        weekly_bills_data = []
        for bill in weekly_bills_qs:
            weekly_bills_data.append({
                'id': bill.id,
                'type': 'weekly',
                'fee_type': bill.fee_type.name,
                'fee_type_id': bill.fee_type_id,
                'collection_frequency': 'WEEKLY',
                'period': f"Week of {bill.week_start} - {bill.week_end}",
                'amount_billed': float(bill.amount_billed),
                'amount_paid': float(bill.amount_paid),
                'balance': float(bill.balance),
                'status': bill.status,
                'due_date': str(bill.week_end),
            })

        all_bills = term_bills_data + weekly_bills_data
        
        # Sort by balance descending (highest owed first)
        all_bills.sort(key=lambda b: b['balance'], reverse=True)

        total_outstanding = sum(b['balance'] for b in all_bills if b['status'] not in ('PAID', 'WAIVED'))
        total_paid = sum(b['amount_paid'] for b in all_bills)
        ...
        return Response({
            ...
            'summary': {
                'total_outstanding': total_outstanding,
                'total_paid': total_paid,
                'online_payments_enabled': getattr(school, 'student_can_pay_online', False),
            },
        })
    ...
    def initiate_payment(self, request):
        """
        POST /fees/student-payments/pay/
        Initiate a Paystack payment for a selected bill.
        
        Body:
        {
            "bill_id": <int>,
            "bill_type": "term" | "weekly",
        ...
        # Check online payments enabled
        err = self._check_online_enabled(school)
        if err:
            return err

        serializer = StudentInitiatePaymentSerializer(data=request.data)
        if not serializer.is_valid():
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

        bill_id = serializer.validated_data['bill_id']
        bill_type = serializer.validated_data.get('bill_type', 'term')
        partial_amount = serializer.validated_data.get('amount')

        # Fetch the bill
        if bill_type == 'term':
            try:
                bill = TermBill.objects.get(pk=bill_id, student=student, school=school)
            except TermBill.DoesNotExist:
                return Response({'error': 'Term bill not found.'}, status=status.HTTP_404_NOT_FOUND)
        elif bill_type == 'weekly':
            try:
        ...
        amount_to_pay = float(partial_amount) if partial_amount else float(bill.balance)
        if amount_to_pay <= 0:
        ...
        reference = f"STU-{bill_type.upper()}-{bill.id}-{uuid.uuid4().hex[:8].upper()}"
        callback_url = f"{getattr(django_settings, 'FRONTEND_URL', '')}/student/payments?ref={reference}"
    ...
    def verify_payment(self, request):
        """
        GET /fees/student-payments/verify/?reference=<ref>
        Verify a Paystack payment and record it if successful.
        """
        import requests as http_requests
        from decimal import Decimal
        ...
        # Extract bill info from reference format: STU-<TYPE>-<id>-<hex>
        try:
            parts = reference.split('-')
            bill_type = parts[1].lower()  # TERM or WEEKLY
            bill_id = int(parts[2])
        except (IndexError, ValueError):
            return Response({'error': 'Invalid reference format.'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        # Idempotency: already recorded
        if FeePayment.objects.filter(paystack_reference=reference, paystack_status='success').exists():
            return Response({'already_recorded': True, 'message': 'Payment was already recorded.'})
        ...
        FeePayment.objects.create(
            ...
            paystack_status='success',
            is_verified=True,
            notes=f'Student online payment via Paystack. Ref: {reference}',
        )

        # Update the bill's amount_paid (balance + status updated in save())
        bill.amount_paid = bill.amount_paid + amount_paid
        bill.save()

        # Update running StudentFee totals
        try:
            sf, _ = StudentFee.objects.get_or_create(
                student=student,
                school=school,
            ...
            logger.warning(f'StudentFee sync after student Paystack verify failed: {_sf_err}')
