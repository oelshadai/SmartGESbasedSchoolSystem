...
class SchoolRegistrationSerializer(serializers.Serializer):
    ...
    def create(self, validated_data):
        try:
            ...
            # Create Subscription row so the subscriptions API works correctly
            try:
                from subscriptions.models import Subscription as Sub, SubscriptionPlan as SubPlan
                plan_obj = SubPlan.objects.filter(plan_type=plan, is_active=True).first()
                Sub.objects.create(
                    school=school,
                    plan=plan_obj,
                    plan_type=plan,
                ...
                logger.info(f"Subscription row created for school {school.id}")
            except Exception as sub_err:
                logger.warning(f"Subscription row creation failed (non-critical): {str(sub_err)}")
